    ...
    for Subj_ID in range(nb_subj):
        ...
        if not SVAD:
            ...
            for views_train, views_val, views_test in zip(views_train_folds, views_val_folds, views_test_folds):
                _, _, _, _, corr_sum_att_list, corr_sum_unatt_list, nb_correct_list, nb_trials_list = utils_unsup_single_enc.iterate(views_train, views_val, views_test, fs, track_resolu, compete_resolu, SVAD=SVAD, MAX_ITER=MAX_ITER, LWCOV=LWCOV, coe=coe, latent_dimensions=latent_dimensions, BOOTSTRAP=BOOTSTRAP, MIXPAIR=MIXPAIR)
