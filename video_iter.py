...
files = get_video_sequences("../../Experiments/data/Two_Obj/Overlay/Pilot_1/")
...
for SEED in args.seeds:
    ...
    file_name = f"{folder_path}nbdisconnected{nb_disconnected}_updatelen{update_len}_predlen{pred_len}_nbestsubj{args.nbestsubj}_hparadata{hparadata[0]}_{hparadata[1]}_hparafeats{hparafeats[0]}_{hparafeats[1]}_evalpara{evalpara[0]}_{evalpara[1]}_SHUFFLE{SHUFFLE}_SEED{SEED}.pkl"
    ...
    est_corr_att_sum, est_corr_unatt_sum = utils_prob.estimate_distribution_corr(eeg_dict, feats_dict, labels_dict, est_subs, fs, hparadata, hparafeats, leave_out_persubj=2, trial_len=update_len, range_into_account=evalpara[0], nb_comp_into_account=evalpara[1])
    ...
    for subj in selected_subjects:
        ...
        print(f"Subject: {subj}, Acc supervised: {acc_sup:.2f}, Acc single: {acc_single}, Acc two: {acc_two}, Acc single unbiased: {acc_single_unbiased}, Acc two unbiased: {acc_two_unbiased}, Acc soft: {acc_soft}")
