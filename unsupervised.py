...
for SEED in args.seeds:
    ...
    for Subj_ID in range(nb_subj):
        file_name = f'{table_path}{Subj_ID}_twoenc{'_trainmin'+str(trainmin) if trainmin is not None else ''}_hankel{str(params_hankel)}_eval{str(evalpara)}_trackresolu{track_resolu}_{'svadresolu' if SVAD else 'mmresolu'}_{compete_resolu}_coe{coe}_nbiter{MAX_ITER}_seed{SEED}{'_lwcov' if LWCOV else ''}{'_bootstrap' if BOOTSTRAP else ''}{'_randinit' if RANDINIT else ''}{'_unbiased' if UNBIASED else ''}{'_mixpair' if MIXPAIR else ''}.pkl'
        print(f'#########Subject: {Subj_ID}#########')
        if not SVAD:
            views_train, views_val, views_test = utils_unsup.prepare_train_val_test_data(Subj_ID, MOD, modal_dict, modal_dict_SO, feat_att_unatt_list, feat_SO_list, params_hankel, fs, trainmin=trainmin)
            model_list, influence_list, mask_list, updated_label_list, rt_list, corr_sum_list, _, nb_correct_train_list, nb_trials_train_list, nb_correct_list, nb_trials_list = utils_unsup.iterate(views_train, views_val, views_test, fs, track_resolu, compete_resolu, SEED, SVAD=SVAD, MAX_ITER=MAX_ITER, LWCOV=LWCOV, coe=coe, SAMEWEIGHT=False, latent_dimensions=latent_dimensions, evalpara=evalpara, BOOTSTRAP=BOOTSTRAP, MIXPAIR=MIXPAIR, TWOENC=False, RANDINIT=RANDINIT, UNBIASED=UNBIASED)
            ...
            with open(file_name, 'wb') as f:
                ...
                res = {'model': model_list, 'influence_list': influence_list, 'mask_list':mask_list, 'updated_label_list':updated_label_list, 'rt_list': rt_list, 'corr_sum_list': corr_sum_list, 'acc_train_list': acc_train_list, 'acc_list': acc_list}
                pickle.dump(res, f)
        else:
            corr_sum_att_folds = []
