...
argparser.add_argument('--label_resolu', type=int, default=30, help='Resolution of the label (in the training set)')
argparser.add_argument('--track_resolu', type=int, default=30, help='Resolution of the tracking influence')
argparser.add_argument('--mm_resolu', type=int, default=30, help='Resolution of the mm task')
...
for SEED in args.seeds:
    ...
    for Subj_ID in range(nb_subj):
        file_name = f'{table_path}{Subj_ID}_{'crossview' if CROSSVIEW else 'featview'}_trackresolu{track_resolu}_truelabelpct_{TRUELABEL_PERCENT}_labelresolu_{label_resolu}_mmresolu_{mm_resolu}coe{coe}_nbiter{MAX_ITER}_seed{SEED}{'_lwcov' if LWCOV else ''}{'_samew' if SAMEWEIGHT else ''}{KEEP_TRAIN_PERCENT if KEEP_TRAIN_PERCENT is not None else ''}.pkl'
        ...
        model_list, influence_list, mask_list, avg_mag_list, rt_list, corr_sum_list, acc_list = utils_unsup.iterate(views_train, views_val, views_test, fs, track_resolu, mm_resolu, L_data, L_feats, MAX_ITER=MAX_ITER, LWCOV=LWCOV, CROSSVIEW=CROSSVIEW, coe=coe, SAMEWEIGHT=SAMEWEIGHT, latent_dimensions=latent_dimensions)
        with open(file_name, 'wb') as f:
            # create a dictionary to save the data
            res = {'model': model_list, 'influence_list': influence_list, 'mask_list':mask_list, 'avg_mag_list': avg_mag_list, 'rt_list': rt_list, 'corr_sum_list': corr_sum_list, 'acc_list': acc_list}
            pickle.dump(res, f)
