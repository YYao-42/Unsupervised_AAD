...
if not TIMEADAPTIVE:
    ...
    for SEED in args.seeds:
        ...
        for Subj_ID in Subj_IDs:
            ...
            file_name = f'{table_path}{Subj_ID}_{'adap' if args.recursive else 'slidingwin'}_singleenc_folds{folds}_hankel{str(params_hankel)}_eval{str(evalpara)}{('_weightpara'+str(weightpara)) if weightpara is not None else ''}{('_poolsize'+str(pool_size)) if pool_size is not None else ''}{'_track_resolu'+str(track_resolu) if args.track_resolu is not None else ''}_compete_resolu{compete_resolu}_seed{SEED}{'_bootstrap' if BOOTSTRAP else ''}{'_ls' if LS else ''}{'_newsplit' if Neetha else ''}{'_initwtrained' if args.initwtrainedmodel else ''}.pkl'
            print(f'#########Subject: {Subj_ID}#########')
            nb_correct_folds = []
            nb_trials_folds = []
            ...
            for i, (views_train, views_test) in enumerate(zip(views_train_folds, views_test_folds)):
                print(f'############Fold: {i}############')
                if args.recursive:
                    _, nb_correct_list, nb_trials_list, model, Rxx, Dxx = utils_unsup_single_enc.recursive(views_train, views_test, fs, track_resolu, compete_resolu, SEED, latent_dimensions=latent_dimensions, weightpara=weightpara, evalpara=evalpara, BOOTSTRAP=BOOTSTRAP, LS=LS, model_init=model_init_folds[i], Rinit=Rinit_folds[i], Dinit=Dinit_folds[i])
                    if args.initwtrainedmodel:
                        model_init_folds[i] = model
                        Rinit_folds[i] = Rxx
                        Dinit_folds[i] = Dxx
                else:
                    _, nb_correct_list, nb_trials_list, model, pool_tensor = utils_unsup_single_enc.sliding_window(views_train, views_test, fs, pool_size, track_resolu, compete_resolu, SEED, latent_dimensions=latent_dimensions, evalpara=evalpara, BOOTSTRAP=BOOTSTRAP, model_init=model_init_folds[i], pool_init=pool_init_folds[i])
                    if args.initwtrainedmodel:
                        model_init_folds[i] = model
